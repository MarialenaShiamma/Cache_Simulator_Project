...
void Parser(int temp,  int Direct_Mapped, int N_Way_Set_Associative, int Fully_Associative, int AddBits, int index, int tag, int offset, int WordsPerBlock, int RamWords, int CacheEntries, int CacheWords, int Way, int WB, int WT, int WA, int NWA, int MCRC, int MCRR, int MCWC, int MCWR, int FIFO, int LRU, int RANDOM){
	...
	struct List_node_DM *FirstNode1, *Pointer1, *tmp1;  //Our first node of the list
	...
	//Create the list if the Architecture is N-Way set Associative
		...
		//We create the nodes
		for(s=1; s <= CacheWords /(WordsPerBlock * Way); s++){
			if (NW_flag == 0){
									...
									if (TagDec == Pointer1->Tag1){
										//Same Block Offset
											if (Pointer1->BlockOffset1[BlockOffsetDec] == Address){
													penalty_cycles = MCRC; //We have to read from the cache
													total_penalty_cycles = (total_penalty_cycles + MCRC); //We have to read from the cache
													hits++;
											...
											//Different Block Offset
												...
												penalty_cycles = (MCRC+MCRR); //We have to read from the cache
								...
								if (Pointer1->Valid1 == 1){
									...
									if (TagDec == Pointer1->Tag1){
										...
										if (Pointer1->BlockOffset1[BlockOffsetDec] == Address){
											...
											if ((( WA == 1) && (NWA == 0)) && ( (WT == 0) && (WB == 1))){
												total_penalty_cycles = total_penalty_cycles + ( MCRC + MCWC); //We read from the cache and we write in the cache
												penalty_cycles = ( MCRC + MCWC); //We read from the cache and we write in the cache
											}
											...
											//Write Through & Write Allocate => Write in Cache and RAM (WT) 
												...
												total_penalty_cycles = total_penalty_cycles + (MCRC + MCWC + MCWR); //We read from the cache, we write in the cache and in RAM too
												penalty_cycles = (MCRC + MCWC + MCWR); //We read from the cache, we write in the cache and in RAM too
											}

											//Write Back & No Write Allocate => Write in Cache and RAM (WT) (BUT you can't write in Cache (WB))
											else if ((( WA == 0) && (NWA == 1)) && ( (WT == 1) && (WB == 0))){
												total_penalty_cycles = total_penalty_cycles + (MCRC + MCWR + MCWC); //NWA means that we don't write in the Cache so we only read from it and write in in RAM and in Cache
												penalty_cycles = penalty_cycles + (MCRC + MCWR + MCWC); //NWA means that we don't write in the Cache so we only read from it and write in in RAM and in Cache
											}
					...
					//The case that our arcitecture is Fully Associative
									...
									if (Pointer2->Tag2 == TagDec){
										...
										if (Pointer2->BlockOffset2[BlockOffsetDec] == Address){
											...
											//Write Through & No Write Allocate => Write in Cache and RAM (WT) (BUT you can't write in Cache (WB))
												...
												total_penalty_cycles = total_penalty_cycles + (MCRC + MCWR + MCWC); //NWA means that we don't write in the Cache so we only read from it and write in in RAM and in Cache
												penalty_cycles = (MCRC + MCWR + MCWC); //NWA means that we don't write in the Cache so we only read from it and write in in RAM and in Cache
											}
						...
						if (inCache == 0){
							...
							if (FIFO == 1){
								...
								RandomNumber = rand() % (CacheEntries-2);
